...
var mySwiper1 = new Swiper('.slide1', {
	...
	on: {
	    slideChangeTransitionStart: function(){
			$('.header-txt li').removeClass('active');
	      	$('.header-txt li').eq(this.activeIndex).addClass('active');	    	
	    },
  	},
...
$('.header-nav').on('touchstart',function(e){
	var touch = e.originalEvent,
		...
		startX = startX - parseInt($('.header-nav').css('left'));
	
	$('.header-nav').on('touchmove',function(e){
		e.preventDefault();
		touch = e.originalEvent.touches[0] || e.originalEvent.changedTouches[0];
		var pos = touch.pageX-startX,
			minpos =  $(document).width() - $('.header-nav').width();
		if(pos > 0){
			pos = 0;
		}else if(pos < minpos){
			pos = minpos;
